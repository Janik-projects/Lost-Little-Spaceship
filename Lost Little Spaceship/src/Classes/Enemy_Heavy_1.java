...
public class Enemy_Heavy_1 extends AnimatedObject {
	...
	public Enemy_Heavy_1(double X, double Y) {
		x = X;
		y = Y;
		dead = false;
		HealthPoints = 22;
	...
	boolean Draw(Mainclass h, Graphics2D g2t) {
		...
		HitBox1.setBounds((int) x + 6, (int) y, h.Enemy_3_1.getWidth() - 12, h.Enemy_3_1.getHeight() - 21);

		if (counter1 < 20) {

			g2t.drawImage(h.Enemy_3_1, (int) x, (int) y, h);
			counter1++;

		} else if (counter1 < 40) {
			g2t.drawImage(h.Enemy_3_2, (int) x, (int) y, h);
			counter1++;

		} else if (counter1 < 60) {
			g2t.drawImage(h.Enemy_3_3, (int) x, (int) y, h);
			counter1++;
		} else {
		...
		for (int i = 0; i < h.allObjects.size(); i++) {
			...
			if (h.allObjects.get(i) instanceof Spaceship1 || h.allObjects.get(i) instanceof Spaceship2
				...
				if (HitBox1.intersects(h.allObjects.get(i).getRect1())
					...
					h.changePlayerHealth(-8);

					h.Explosions
							.add(new ExplosionSmall(x + h.Enemy_3_1.getWidth() / 2 - h.Explosion_Small_1.getWidth() / 2,
									y + h.Enemy_3_1.getHeight() / 2));
					h.Explosions.add(
							new ExplosionMedium(x + h.Enemy_3_1.getWidth() / 2 - h.Explosion_medium_0.getWidth() / 2,
									y + h.Explosion_medium_0.getHeight() / 2));
		...
		if (dead) {
			...
			if(PSound){
				h.Explosions.add(new EnemyShipExplosion(x, y, 3));
				
				
			
			h.Soundclass.playSound("explosion.wav", h.Volume - 10);
			
			h.Soundclass.playSound("enemy3_exp.wav", h.Volume );
			h.Soundclass.playSound("enemy3_exp2.wav", h.Volume);
	...
	private void WeaponControl(Mainclass h, int chance1, int chance2, int chance3) {
		...
		if (r1.nextInt(chance1) < 10) {
			...
			h.EnemyWeapons.add(new WeaponShotEnemy(x + 27, y + 111, 1, 0, 0));
			h.EnemyWeapons.add(new WeaponShotEnemy(x + 69, y + 111, 1, 0, 0));

			h.EnemyWeapons.add(new WeaponShotEnemy(x + 15, y + 69, 1, 0, 0));
			h.EnemyWeapons.add(new WeaponShotEnemy(x + 81, y + 69, 1, 0, 0));

		}

		if (r1.nextInt(chance2) < 10) {
			
			if(PSound)
			h.Soundclass.playSound("rocket_fire1.wav", h.Volume + 12);
			
			h.EnemyWeapons.add(new WeaponShotEnemy(x + 9, y + 102, 3, h.enemyRocketCounter, 0));
			h.enemyRocketCounter++;
			h.EnemyWeapons.add(new WeaponShotEnemy(x + 75, y + 102, 3, h.enemyRocketCounter, 0));
			h.enemyRocketCounter++;
		}
