...
public class Enemy_Medium_1 extends AnimatedObject {
	...
	public Enemy_Medium_1(double X, double Y, long IDthis) {
		x = X;
		y = Y;
		dead = false;
		HealthPoints = 12;
	...
	boolean Draw(Mainclass h, Graphics2D g2t) {
		...
		if(y < h.height && x + h.Enemy_2_1.getWidth() > 0 && x <h.width){
			PSound = true;
		} else {
			PSound = false;
		}
		
		HitBox1.setBounds((int) x + 3, (int) y, h.Enemy_2_1.getWidth() - 6, h.Enemy_2_1.getHeight() - 6);

		if (counter1 < 20) {

			g2t.drawImage(h.Enemy_2_1, (int) x, (int) y, h);
			counter1++;

		} else if (counter1 < 40) {
			g2t.drawImage(h.Enemy_2_2, (int) x, (int) y, h);
			counter1++;

		} else if (counter1 < 60) {
			g2t.drawImage(h.Enemy_2_3, (int) x, (int) y, h);
			counter1++;
		} else {
		...
		for (int i = 0; i < h.allObjects.size(); i++) {
			...
			if (h.allObjects.get(i) instanceof Spaceship1 || h.allObjects.get(i) instanceof Spaceship2
				...
				if (HitBox1.intersects(h.allObjects.get(i).getRect1())
					...
					h.changePlayerHealth(-4);

					h.Explosions
							.add(new ExplosionSmall(x + h.Enemy_2_1.getWidth() / 2 - h.Explosion_Small_1.getWidth() / 2,
									y + h.Enemy_2_1.getHeight() / 2));
		...
		if (dead) {
			...
			for (int k = 0; k < h.EnemyWeapons.size(); k++) {

				if (h.EnemyWeapons.get(k).getID() == ID) {

					if (h.EnemyWeapons.get(k).getType() == 2) {

						h.EnemyWeapons.remove(k);
						k--;
					}
			...
			h.Explosions.add(new EnemyShipExplosion(x, y, 2));
	...
	private void WeaponControl(Mainclass h, int chance1, int chance2, int chance3) {
		...
		if (r1.nextInt(chance1) < 10) {
			...
			h.EnemyWeapons.add(new WeaponShotEnemy(x + 3, y + 57, 1, 0, 0));
			h.EnemyWeapons.add(new WeaponShotEnemy(x + 33, y + 57, 1, 0, 0));
		...
		if (r1.nextInt(chance2) < 10) {
			...
			if(PSound)
			h.Soundclass.playSound("enemy_medium.wav", h.Volume);
			
			h.EnemyWeapons.add(new WeaponShotEnemy(x + 19, y + 42, 2, 0, ID));
		}
	...
	private void moveLaser(Mainclass h) {
		for (int k = 0; k < h.EnemyWeapons.size(); k++) {

			if (h.EnemyWeapons.get(k).getID() == ID) {

				h.EnemyWeapons.get(k).setX(this.x + 19);
				h.EnemyWeapons.get(k).setY(this.y + 42);
